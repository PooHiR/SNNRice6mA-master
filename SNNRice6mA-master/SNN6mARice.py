...
def analyze(temp, OutputDir):
    ...
    training_result, validation_result, testing_result = temp;

    file = open(OutputDir + '/performance.txt', 'w')

    index = 0
    for x in [training_result, validation_result, testing_result]:
...
def chunkIt(seq, num): #将打乱的序列输入进来，然后avg = 88，每一次out.append()的数据是：[88个41行4列的数据],out里面有10组这样的数据
...
def prepareData(PositiveCSV, NegativeCSV):
    ...
    print(type(Positive_X),type(Positive_y))
...
def getMode():
    ...
    # print("normalizationLayer,",normalizationLayer.shape)
    poolingLayer = MaxPooling1D(pool_size = 4)(normalizationLayer)
    # print("poolingLayer.shape",poolingLayer.shape)
    flattenLayer = Flatten()(poolingLayer)
    # print("flattenLayer.shape",flattenLayer.shape)

    dropoutLayer = Dropout(0.25)(flattenLayer)
    # print("dropoutLayer.shape",dropoutLayer.shape)
    ...
    # print("denseLayer,shape",dropoutLayer.shape)
    outLayer = Dense(1, activation='sigmoid')(denseLayer)
    # print("outLayer.shape",outLayer.shape)
...
def calculateScore(X, y, model):
    ...
    #     lossValue = losses.binary_crossentropy(y_true, y_pred,from_logits=True).eval()
...
def funciton(PositiveCSV, NegativeCSV,  folds,OutputDir=None):

    Positive_X, Positive_y, Negitive_X, Negitive_y = prepareData(PositiveCSV, NegativeCSV)
    print(type(Positive_X))

    random.shuffle(Positive_X);  # 随机打乱，但是标签都是1
    random.shuffle(Negitive_X);  # 随机打乱，但是标签都是0
    ...
    for test_index in range(folds):
        ...
        valid_X = np.concatenate((Positive_X_Slices[validation_index],Negative_X_Slices[validation_index])) # x正例和反例放在一起
        valid_y = np.concatenate((Positive_y_Slices[validation_index],Negative_y_Slices[validation_index])) # y也是正例和反例放在一起
        ...
        for i in range(0, folds):#这里开始把训练集变成8成
        ...
        # main2(train_X, train_y, test_X, test_y, valid_X, valid_y)
        model = getMode();
    ...
    # #
        history = model.fit(train_X, train_y, batch_size = 64, epochs = 100, validation_data = (valid_X, valid_y),callbacks = [model_check, early_stopping,reduct_L_rate]);
        print("*"*100)
...
def main():
    ...
    parser.add_argument("--positive", type=str, help="positive 6mA csv", required=False)
    parser.add_argument("--negative", type=str, help="negative 6mA csv", required=False)
    ...
    PositiveCSV = r'E:\6mA\SNNRice6mA-master\SNNRice6mA-master\6mA_data\Rice_Chen\Positive.txt'
    NegativeCSV = r'E:\6mA\SNNRice6mA-master\SNNRice6mA-master\6mA_data\Rice_Chen\Negative.txt'
    ...
    funciton(PositiveCSV, NegativeCSV,  10,OutputDir);
    # model = getMode();
